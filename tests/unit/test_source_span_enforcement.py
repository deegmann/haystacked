...
def test_U_SS_04_l0_fabricated_value_with_self_consistent_fake_quote_nulls():
    ...
    required_lifting_height_mm=4.8 with the plausible quote 'The maximum lift
...
def test_U_SS_06_l0_german_decimal_comma_survives():
    ...
    """
    key = "required_min_aisle_width_mm"
...
def test_U_SS_07_l0_pdf_glyph_artifact_does_not_merge_numbers():
    ...
    min_criteria = {"required_operating_temp_min_c": 10, "required_operating_temp_min_c_source": quote}
    max_criteria = {"required_operating_temp_max_c": 30, "required_operating_temp_max_c_source": quote}

    min_result = _apply(min_criteria, document_text=document)
    max_result = _apply(max_criteria, document_text=document)

    assert min_result["required_operating_temp_min_c"] == 10, "10 must ground despite the glyph artifact"
    assert max_result["required_operating_temp_max_c"] == 30, "30 must ground despite the glyph artifact"
...
def test_U_SS_11_documented_residual_gradient_collision_case():
    ...
    investigation: CompanyX's fabricated required_max_gradient_pct=10 was
