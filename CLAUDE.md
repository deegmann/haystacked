...
**Safety Rule — before any `git checkout HEAD -- <file>`, `git reset --hard`, or other destructive revert:** if there are uncommitted changes anywhere in the working tree, push the entire current state to a safety branch first, even if the purpose of those changes is unclear:
```bash
git checkout -b safety/YYYY-MM-DD-<reason>
git add -A
git commit -m "Safety: working tree before revert — <reason>"
git checkout main
# now safe to revert
```
Never destroy uncommitted changes without first preserving them. This is especially critical for the AP0 xlsx and any generated config, but applies to all files.
...
`source_is_grounded(value, source, document, unit="")` (`src/json_repair.py`) checks whether the LLM's self-reported quote is actually anchored in the real extracted PDF text. Two binary conditions must both hold: (1) the value's digit-string must occur somewhere in the real document (anchor), and (2) at least one distinctive content word from the quote must appear within 80 characters of an anchor occurrence (co-location). Pure function — no domain knowledge, no field names.

The anchor check (1) treats the exact-scale target (`value` itself) as always sufficient, unchanged. A ×1000/÷1000 unit-scale-converted target (e.g. `value=10` matching a document's `10000`) is now **unit-gated**: it only counts if `unit` is a key in the generic `_METRIC_PREFIX_SCALE` table (`mm`/`cm`/`m`/`km`/`g`/`kg`/`t` — no field names, pure SI scale factors) AND a metric-prefix unit token sits within 3 characters of the matched document number AND that unit, applied to the document's number, dimensionally resolves to the same real-world quantity as `value` under `unit` (`enforce_source_spans()` passes `unit=units.get(key, "")`, i.e. the field's AP0 Unit column value, at its `source_is_grounded()` call site). If `unit` is empty or not a `_METRIC_PREFIX_SCALE` key, converted-scale targets anchor unconditionally — a deliberate carve-out for non-metric units (`%`, `°C`, `h`, `kW`, ...) this table has no opinion about. This closes a structural gap where a round value like `10000` implicitly included a bare `10` in its anchor set, which could anchor against unrelated document boilerplate (e.g. a `"10 years"` clause or an unrelated `"width mm 10"` table cell) — the residual risk `test_U_SS_11` (`tests/unit/test_source_span_enforcement.py`, 2026-06-16) flagged and monitored, which materialized on Dragonfly's `required_lifting_height_mm` field; see `test_U_SG_15`–`test_U_SG_21` in `tests/unit/test_source_is_grounded.py`.
